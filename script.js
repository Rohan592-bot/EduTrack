...
function printStudentReport() {
  const body  = document.getElementById('pm-body').innerHTML;
  ...
  w.document.write(`<!DOCTYPE html><html><head>
    ...
    <style>
      :root{--bg-base:#fff;--bg-raised:#f9fafb;--bg-overlay:#f3f4f6;--border:#e5e7eb;--border-md:#d1d5db;
        --text-hi:#111827;--text-md:#6b7280;--text-lo:#9ca3af;
        --amber:#d97706;--amber-dim:rgba(217,119,6,.1);--green:#16a34a;--green-dim:rgba(22,163,74,.1);
        --blue:#2563eb;--blue-dim:rgba(37,99,235,.1);--red:#dc2626;--red-dim:rgba(220,38,38,.1);
        --purple:#7c3aed;--purple-dim:rgba(124,58,237,.1);--radius-sm:6px;--radius:10px;--radius-lg:14px;}
      *{box-sizing:border-box;margin:0;padding:0}
      body{background:#fff;color:#111827;font-family:'Inter',sans-serif;padding:32px;font-size:13px;line-height:1.6}
      .print-header{border-bottom:2px solid #111827;padding-bottom:14px;margin-bottom:20px}
      .print-header h1{font-family:'Sora',sans-serif;font-size:1.4rem;font-weight:700;color:#111827}
      .print-header p{font-size:.8rem;color:#6b7280;margin-top:4px}
      .print-footer{margin-top:28px;padding-top:12px;border-top:1px solid #e5e7eb;font-size:.72rem;color:#9ca3af;text-align:center}
      .pm-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px}
      .info-panel{background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:14px}
      .info-panel h4{font-size:.65rem;font-weight:600;text-transform:uppercase;letter-spacing:.09em;color:#9ca3af;margin-bottom:10px}
      .info-row{display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #f3f4f6;font-size:.78rem}
      .info-row:last-child{border-bottom:none}
      .info-key{color:#6b7280}
      .info-val{font-weight:600;font-family:'JetBrains Mono',monospace;font-size:.76rem;color:#111827}
      .bar-row{display:flex;align-items:center;gap:9px;margin-bottom:7px}
      .bar-lbl{font-size:.68rem;color:#6b7280;width:44px;flex-shrink:0;font-family:'JetBrains Mono',monospace}
      .bar-track{flex:1;height:5px;background:#e5e7eb;border-radius:99px;overflow:hidden}
      .bar-fill{height:100%;border-radius:99px}
      .bar-val{font-family:'JetBrains Mono',monospace;font-size:.7rem;width:32px;text-align:right}
      .panel{background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:14px;margin-top:12px}
      .panel-head{margin-bottom:10px;font-weight:600;font-size:.82rem;color:#374151}
      ...
      .pill-prog{background:rgba(217,119,6,.1);color:#d97706}
      .pill-female{background:rgba(124,58,237,.1);color:#7c3aed}
      .pill-male{background:rgba(37,99,235,.1);color:#2563eb}
      .tier-distinction{background:rgba(22,163,74,.1);color:#16a34a;border:1px solid rgba(22,163,74,.2);padding:2px 8px;border-radius:99px;font-size:.63rem;font-weight:600}
      .tier-merit{background:rgba(37,99,235,.1);color:#2563eb;border:1px solid rgba(37,99,235,.2);padding:2px 8px;border-radius:99px;font-size:.63rem;font-weight:600}
      .tier-pass{background:rgba(217,119,6,.1);color:#d97706;border:1px solid rgba(217,119,6,.2);padding:2px 8px;border-radius:99px;font-size:.63rem;font-weight:600}
      .tier-atrisk{background:rgba(220,38,38,.1);color:#dc2626;border:1px solid rgba(220,38,38,.2);padding:2px 8px;border-radius:99px;font-size:.63rem;font-weight:600}
      canvas{display:none}
    </style></head><body>
    ...
    ${body}
    <div class="print-footer">Generated by EduTrack · ${new Date().toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})}</div>
  </body></html>`);
  w.document.close();
